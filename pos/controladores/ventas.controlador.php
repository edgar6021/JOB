...
class ControladorVentas{
    ...
    if(isset($_POST["impresion"])){
        ...
        // Opción para impresora o PDF
        if ($_POST["impresion"] == "impresora") {
            $impresora = "epson20"; // Configurar según la impresora
        ...
        } elseif ($_POST["impresion"] == "pdf") {

            require('fpdf.php');
            $pdf = new FPDF();
            $pdf->AddPage();
            $pdf->SetFont('Arial', 'B', 12);
            $pdf->Cell(40, 10, 'Inventory System');
            $pdf->Ln();
            $pdf->Cell(40, 10, 'Fecha: '.date("Y-m-d H:i:s"));
            $pdf->Ln();
            $pdf->Cell(40, 10, 'Factura N: '.$_POST["nuevaVenta"]);
            $pdf->Ln();

            foreach ($listaProductos as $key => $value) {
                $pdf->Cell(0, 10, $value["descripcion"].' - $'.number_format($value["precio"], 2), 0, 1);
            }

            $pdf->Ln();
            $pdf->Cell(0, 10, 'Total: $'.number_format($_POST["totalVenta"], 2), 0, 1);
            $pdf->Output('I', 'factura.pdf'); // 'I' para mostrar en el navegador, 'D' para descargar directamente
        }
    }
...
public function ctrDescargarReporte() {
    ...
    if (isset($_GET["reporte"])) {
        ...
        // Cambiamos utf8_decode() por mb_convert_encoding()
        ...
        foreach ($ventas as $row => $item) {
            ...
            echo mb_convert_encoding("<tr>
                    <td style='border:1px solid #eee;'>" . (isset($item["codigo"]) ? $item["codigo"] : '') . "</td>
